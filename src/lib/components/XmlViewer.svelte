    ...
    <rsm:SupplyChainTradeTransaction>
      <ram:ApplicableHeaderTradeAgreement>
        <ram:SellerTradeParty>
          ...
          ${data.sender.taxId ? `<ram:SpecifiedTaxRegistration>
            <ram:ID schemeID="FC">${escapeXml(data.sender.taxId)}</ram:ID>
          </ram:SpecifiedTaxRegistration>` : ''}
          ${data.sender.ustId ? `<ram:SpecifiedTaxRegistration>
      ...
      <ram:ApplicableHeaderTradeDelivery>
        ${data.metadata.deliveryDate ? `<ram:ActualDeliverySupplyChainEvent>
      ...
      <ram:ApplicableHeaderTradeSettlement>
        ...
        ${data.items.map((item, index) => `<ram:SpecifiedTradeSettlementLineMonetarySummation>
          <ram:LineTotalAmount>${(item.quantity * item.unitPrice).toFixed(2)}</ram:LineTotalAmount>
        </ram:SpecifiedTradeSettlementLineMonetarySummation>`).join('\n      ')}
      ...
      ${data.items.map((item, index) => `<ram:IncludedSupplyChainTradeLineItem>
  ...
  <div class="xml-viewer">
    ...
    <div class="xml-footer">
      <div class="validation-status">
        ...
        <span>Schema-Validierung: Struktur korrekt</span>
      </div>
      <button class="btn btn-secondary copy-btn" on:click={() => navigator.clipboard.writeText(xmlContent)}>
        📋 XML kopieren
      </button>
    </div>
  </div>
